...
export function SetEventName(eventName) {
  ...
  export function UpdateLeaderboard(leaderboardData) {
    ...
    if (leaderboardData.length > 0) {
      SetFirstPlaceRacerNameAndTime(leaderboardData[0].username, GetFormattedLapTime(leaderboardData[0].fastestLapTime, true));
    }
    if (leaderboardData.length > 1) {
      SetSecondPlaceRacerNameAndTime(leaderboardData[1].username, GetFormattedLapTime(leaderboardData[1].fastestLapTime, true));
    }
    if (leaderboardData.length > 2) {
      SetThirdPlaceRacerNameAndTime(leaderboardData[2].username, GetFormattedLapTime(leaderboardData[2].fastestLapTime, true));
    }
    if (leaderboardData.length > 3) {
      SetFourthPlaceRacerNameAndTime(leaderboardData[3].username, GetFormattedLapTime(leaderboardData[3].fastestLapTime, true));
    }
  }
  ...
  export function GetLeaderboardDataSorted(entries) {
    ...
    entries.sort(function(a,b) {return (a.fastestLapTime > b.fastestLapTime) ? 1 : ((b.fastestLapTime > a.fastestLapTime) ? -1 : 0);} );
