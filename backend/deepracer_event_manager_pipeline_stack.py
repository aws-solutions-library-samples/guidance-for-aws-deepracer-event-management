...
CDK_VERSION = "2.54.0"  # other possible options: latest
...
class InfrastructurePipelineStage(Stage):
    def __init__(
        ...
        **kwargs,
        ...
        stack = CdkDeepRacerEventManagerStack(
            self, "infrastructure", email=email, env=env
        )
...
class CdkServerlessCharityPipelineStack(Stack):
    def __init__(
        self, scope: Construct, construct_id: str, branchname: str, email: str, **kwargs
    ) -> None:
        super().__init__(scope, construct_id, **kwargs)
        ...
        s3_repo_bucket_parameter_store = (
            ssm.StringParameter.from_string_parameter_attributes(
                self, "S3RepoBucketValue", parameter_name="/drem/S3RepoBucket"
            )
        )

        s3_repo_bucket = s3.Bucket.from_bucket_arn(
            self, "S3RepoBucket", s3_repo_bucket_parameter_store.string_value
        )
        ...
        infrastructure = InfrastructurePipelineStage(
            self, "drem-backend-" + branchname, env, branchname, email
        )
