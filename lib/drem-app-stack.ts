...
export class DeepracerEventManagerStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: DeepracerEventManagerStackProps) {
    ...
    // Direct-invoke Lambda the car calls after SSM registration; the
    // physical function name is generated by CDK and surfaced here so
    // it can be threaded through the amplify config into the install
    // script's CLI args. See `lib/lambdas/register_car_serial/`.
    new cdk.CfnOutput(this, 'registerCarSerialFunctionName', {
      value: carManager.registerCarSerialFunctionName,
    });
