...
def get_sensors(model_metatdata_json):
    ...
    Args:
        model_metatdata_json (dict): JSON object with contents of the model
                metadata file.
    ...
    try:
        ...
        if constants.ModelMetadataKeys.SENSOR in model_metatdata_json:
            ...
            if all(
                [
                    constants.SensorInputTypes.has_member(sensor_name)
                    for sensor_name in sensor_names
                ]
            ):
                sensors = [
                    constants.SensorInputTypes[sensor_name.upper()].value
                    for sensor_name in sensor_names
                ]
            else:
                return (
                    2,
                    (
                        "The sensor configurations of your vehicle and trained model"
                        " must match"
                    ),
                    [],
                )
        ...
        return (
            1,
            f"Error while getting sensor names from model_metadata.json: {exc}",
            [],
        )


def get_training_algorithm(model_metadata_json):
    """Helper method that returns the corresponding enum value for the training
       algorithm in the model_metadata json of the model selected.

    Args:
        model_metadata_json (dict): JSON object with contents of the model
                metadata file.
    ...
    try:
        ...
        if constants.ModelMetadataKeys.TRAINING_ALGORITHM in model_metadata_json:
            training_algorithm_value = model_metadata_json[
                constants.ModelMetadataKeys.TRAINING_ALGORITHM
            ]
            if constants.TrainingAlgorithms.has_member(training_algorithm_value):
                training_algorithm = constants.TrainingAlgorithms[
                    training_algorithm_value.upper()
                ]
            else:
            ...
            print(
                "No training algorithm key in model_metadata_file. Defaulting to"
                " clipped_ppo."
            )
        ...
        return (
            1,
            f"Error while getting training algorithm model_metadata.json: {exc}",
            "",
        )
...
def load_lidar_configuration(sensors, model_metadata):
    ...
    Returns:
        tuple: Tuple of (error_code, error_message, dictionary with model lidar
               configuration corresponding the preprocessing done on LiDAR data during
               model training).
    """
    try:
        # Set default values in case the 'lidar configuration' is not defined in
        # model_metadata.json
        ...
        model_lidar_config[constants.ModelMetadataKeys.USE_LIDAR] = sensors and (
            constants.SensorInputTypes.LIDAR in sensors
            or constants.SensorInputTypes.SECTOR_LIDAR in sensors
        )
        # Load the lidar configuration if the model uses lidar and has custom lidar
        # configurations
        if (
        ...
        return (
            1,
            f"Unable to connect to device with current LiDAR configuration: {exc}",
            {},
        )
