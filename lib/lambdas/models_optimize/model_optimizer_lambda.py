...
The lambda is a mash-up of the code provided by the aws-deepracer-model-optimizer-pkg and the 
aws-deepracer-systems-pkg, allowing for the optimization of models outside of the DeepRacer
...
class ModelOptimizer:
    """Lambda responsible for running the Intel OpenVino model optimizer for the DeepRacer models.
    """
    def __init__(self):
        """Create a ModelOptimizerLambda.
        """
        logger.info("ModelOptimizerLambda started")
    ...
    def optimize(self, model_name):
        ...
        model_metadata_file_path = os.path.join(model_folder, constants.MODEL_METADATA_NAME)

        if not os.path.isfile(model_metadata_file_path):
            logger.error(f"Metadata file not found: {model_metadata_file_path}")
            return 1, f"Metadata file not found: {model_metadata_file_path}", None

        logger.info(f"read model_metadata_file from {model_metadata_file_path}...")
        err_code, err_msg, model_metadata_content = \
            model_metadata_file_utils.read_model_metadata_file(model_metadata_file_path)

        if err_code != 0:
            logger.error("Error while getting sensor names from "
        ...
        if err_code != 0:
            logger.error("Error while getting sensor names from "
                                f"{model_metadata_file_path}: {err_msg}")
            return err_code, err_msg, None
        logger.info("Sensor names read from "
        ...
        if err_code != 0:
            logger.error(f"Error while getting training algorithm from "
                                f"{model_metadata_file_path}: {err_msg}")
            return err_code, err_msg, None
        logger.info("Training algorithm read from "
        ...
        if err_code != 0:
            logger.error("Error while getting LiDAR configuration from "
                                f"{model_metadata_file_path}: {err_msg}")
            return err_code, err_msg, None
        logger.info("LiDAR configuration read from "
                            f"{model_metadata_file_path}: {model_lidar_config}")

        try:
            aux_param = {"--fuse": "OFF", "--img-format": constants.APIDefaults.IMG_FORMAT}
            error_code, artifact_path = self.optimize_tf_model("{}/agent/model".format(model_name),
            ...
            if error_code == 0:
                logger.info(f"Optimized artifact available in : {artifact_path}")
                return error_code, "", artifact_path

        except Exception as ex:
            logger.error(f"Error while optimizing model: {ex}")
            return 1, "Error"
    ...
    def run_optimizer(self, mo_path, common_params, platform_parms):
        ...
        logger.info(f"Model optimizer command: {cmd}")
        ...
        while retry_count <= constants.MAX_OPTIMIZER_RETRY_COUNT:
            logger.info(f"Optimizing model: {retry_count} of "
            ...
            logger.error(f"Model optimizer error info: {std_err}")
            retry_count += 1
    ...
    def set_platform_param(self, platform_param, aux_inputs):
        ...
        """
        logger.info(f"aux_inputs: {aux_inputs} ")
        set_paltform_params = {}
...
def lambda_handler(event, context):
    ...
    if error_code == 0:
        ret_val = dict()
        ret_val['artifact_path'] = artifact_path
    else:
        ret_val = dict()
        ret_val['error_code'] = error_code
        ret_val['error_msg'] = error_msg
