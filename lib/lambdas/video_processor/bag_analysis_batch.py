...
def usage():
    print(
        "Usage: python bag_analysis_batch.py --input_dir <directory> --output_dir <directory> --models_dir <models_directory> [--codec <codec>] [--frame_limit <limit>] [--describe] [--relative_labels] [--background] [--pattern <pattern>]"
    )
    sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        "--input_dir", help="The directory containing the bag files", required=True
    )
    parser.add_argument(
        "--output_dir", help="The directory to save the videos", required=True
    )
    parser.add_argument(
        "--models_dir", help="The directory containing the model files", required=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-background",
        dest="background",
        help="Do not add a background to the video",
        action="store_false",
    )
    ...
    for file in os.listdir(input_dir):
        ...
        if (
            ...
            and os.path.isdir(bag_path)
            and not os.path.commonpath([output_dir, bag_path]).startswith(output_dir)
        ):
