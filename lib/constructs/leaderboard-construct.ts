...
export class Leaderboard extends Construct {
    constructor(scope: Construct, id: string, props: LeaderboardProps) {
        ...
        const laps_data_source = props.appsyncApi.api.addLambdaDataSource("lapsDataSource", laps_lambda)
        ...
        // TimeKeeper methods
        props.appsyncApi.schema.addQuery("getAllRacers",
            new ResolvableField({
                returnType: user_object_type.attribute({ isList: true }),
                dataSource: laps_data_source,
            })
        )
        props.appsyncApi.schema.addMutation("addRace",
            new ResolvableField({
                args: {
        ...
        // Event Admin methods
        props.appsyncApi.schema.addQuery("getRacesForUser",
            new ResolvableField({
                args: {
                    "username": GraphqlType.string({ isRequired: true }),
        ...
        props.appsyncApi.schema.addMutation("updateOverlayInfo",
            new ResolvableField({
                args: {
                    "eventId": GraphqlType.string({ isRequired: true }),
                    "username": GraphqlType.string({ isRequired: true }),
        ...
        props.appsyncApi.schema.addSubscription("onNewOverlayInfo",
            new ResolvableField({
                // args:{
                //     "eventId": GraphqlType.id({isRequired: true}),
                // },
