...
class CarManager extends constructs_1.Construct {
    ...
    constructor(scope, id, props) {
        ...
        carStatusUpdateHandler.addToRolePolicy(new iam.PolicyStatement({
        ...
        const definition = describeInstanceInformationTask.next(status_update_job).next(new stepFunctions.Choice(this, "Job done?")
            .when(stepFunctions.Condition.isPresent("$.NextToken"), describeInstanceInformationTask)
            .otherwise(succeed_job));
        const car_status_update_SM = new stepFunctions.StateMachine(this, "CarStatusUpdater", { definition: definition, timeout: aws_cdk_lib_1.Duration.minutes(3) });
        ...
        car_activation_handler.addToRolePolicy(new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            actions: [
                "iam:PassRole",
        ...
        const car_activation_data_source = props.appsyncApi.api.addLambdaDataSource("car_activation_data_source", car_activation_handler);
        // Define API Schema
        ...
        // Event methods
        props.appsyncApi.schema.addMutation("carActivation", new awscdk_appsync_utils_1.ResolvableField({
            args: {
        ...
        cars_function_handler.addToRolePolicy(new iam.PolicyStatement({
            effect: iam.Effect.ALLOW,
            actions: [
        ...
        props.appsyncApi.schema.addQuery("carsOnline", new awscdk_appsync_utils_1.ResolvableField({
            args: {
        ...
        props.appsyncApi.schema.addMutation("carUpdates", new awscdk_appsync_utils_1.ResolvableField({
            args: {
        ...
        props.appsyncApi.schema.addMutation("carDeleteAllModels", new awscdk_appsync_utils_1.ResolvableField({
            args: {
                "resourceIds": awscdk_appsync_utils_1.GraphqlType.string({ isList: true, isRequired: true }),
            },
            returnType: awscdk_appsync_utils_1.GraphqlType.awsJson(),
            dataSource: cars_data_source,
        }));
        props.appsyncApi.schema.addMutation("carSetTaillightColor", new awscdk_appsync_utils_1.ResolvableField({
            args: {
                "resourceIds": awscdk_appsync_utils_1.GraphqlType.string({ isList: true, isRequired: true }),
                "selectedColor": awscdk_appsync_utils_1.GraphqlType.string({ isList: false, isRequired: true }),
            },
            returnType: awscdk_appsync_utils_1.GraphqlType.awsJson(),
            dataSource: cars_data_source,
        }));
        props.appsyncApi.schema.addQuery("availableTaillightColors", new awscdk_appsync_utils_1.ResolvableField({
            returnType: awscdk_appsync_utils_1.GraphqlType.awsJson(),
            dataSource: cars_data_source,
        }));
        // All Methods...
