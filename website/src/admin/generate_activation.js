...
class AdminActivation extends Component {
  ...
  getActivation = async () => {
    ...
    this.setState({
      ...
      UpdateCommand: 'curl -O ' + window.location.protocol + '://' + window.location.hostname + '/manual_update.sh && chmod +x ./manual_update.sh && sudo ./manual_update.sh -h ' + this.state.hostname + ' -c '+ response['ActivationCode'] +' -i '+ response['ActivationId'] +' -r '+ response['region'] +'',
      loading: false,
    });
    //return response
  }
