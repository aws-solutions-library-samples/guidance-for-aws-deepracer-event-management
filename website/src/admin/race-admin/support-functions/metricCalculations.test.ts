...
// Note: Test data uses { time, resets, isValid } to match the actual GraphQL API response shape.
// The domain Lap type has { lapTime, resetCount } but there is no transformation layer,
// so at runtime the API field names are what metricCalculations.ts actually receives.
