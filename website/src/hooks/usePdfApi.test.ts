...
function fakeSubscription() {
  ...
  return {
    ...
    emitNull: () => handlers.forEach((h) => h.next({ value: { data: { onPdfJobUpdated: null } } })),
  };
}
...
describe('usePdfApi', () => {
  ...
  test('null subscription payload is tolerated — hook still queries getPdfJob', async () => {
    ...
    await waitFor(() => {
      expect(Object.values(result.current.jobs).some((j) => j.status === 'SUCCESS')).toBe(true);
    });
  });
  ...
  test('immediate placeholder notification — pushed before mutation resolves, then swapped for jobId-keyed one', async () => {
    ...
    expect(
      dispatchMock.mock.calls.some(
        ([action, payload]) => action === 'ADD_NOTIFICATION' && payload?.id === 'pdf-j-imm'
      )
    ).toBe(true);
  });
  ...
  test('placeholder notification is dismissed when the mutation throws', async () => {
    ...
    const { result } = renderHook(() => usePdfApi());
    await act(async () => {
      await expect(result.current.generatePdf({ eventId: 'e-1', type: 'PODIUM' })).rejects.toThrow(
        'AppSync 500'
      );
    });
